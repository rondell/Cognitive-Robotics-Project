...
int WriteAnimation(plotparam *PlotParam, int Width, int Height,
    const char *OutputFile,int vid_frame)
{
...
int main(int argc, char *argv[])
{
    ...
    image data_Phi;
    //MODIFIED BY GARO
    //try to set the prog suitable to accept more then an image
   ...
   cont  = 0;
   int tmp;
    while(fgets(tmp_str,50,fInputImg))
    {
     
     Param.InputFile=tmp_str;
    ...
    if(!WriteAnimation(&PlotParam, f.Width, f.Height, Param.OutputFile,cont))
        goto Catch;
